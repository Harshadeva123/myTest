...
class ResponseAnalysisController extends Controller
{
    public function index(Request $request)
    {
        ...
        if (Auth::user()->officeStaff != null && count(Auth::user()->officeStaff->officeGramasewa()->get()) > 0) {
            $assignedDivisions = Auth::user()->officeStaff->officeGramasewa()->pluck('idgramasewa_division')->toArray();
        ...
        $gramasewaDivisions = GramasewaDivision::where('status', 1)->get();
        return view('analysis.response_analysis')->with(['gramasewaDivisions' => $gramasewaDivisions, 'title' => 'Response Analysis', 'responses' => $responses]);
    }
