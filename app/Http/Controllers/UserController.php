...
class UserController extends Controller
{
    ...
    public function store(Request $request){
        ...
        if(UserTitle::find(intval($request['title']))->gender != null && UserTitle::find(intval($request['title']))->gender != $request['gender'] ){
    ...
    public function view(Request $request)
    {
        ...
        if (!empty($searchText)) {
            if($searchCol == 1){
                $query = $query->where('fName', $searchText);

            }
            else if($searchCol == 2){
                $query = $query->where('lName', $searchText);
        ...
        return view('user.view_users', ['title' =>  __('View User'), 'users' => $users, 'userRoles' => $userRoles,'offices'=>$offices]);
    }
    ...
    public function generateReferral($uid){
        ...
        $referral =  mt_rand(10, 99).$name[0].substr(str_shuffle($permitted_chars), 0, 2).substr(str_shuffle($user->office->office_name), 0, 2);
