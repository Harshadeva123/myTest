    ...
    public function store(Request $request)
    {
        ...
        if ($user != null) {
            foreach ($gramasewaDivisions as $gramasewaDivision) {
                ...
                $isExist = StaffGramasewaDivision::where('idoffice', Auth::user()->idoffice)->where('idgramasewa_division', $gramasewaDivision)->where('status', 1)->first();

                if ($isExist != null) {
                    return response()->json(['errors' => ['error' => '`' . GramasewaDivision::find(intval($gramasewaDivision))->name_en . '` gramasewa division has already assigned to `' . strtoupper(User::find($isExist->staff->idUser)->fName) . '``.']]);
            ...
            foreach ($gramasewaDivisions as $gramasewaDivision) {
                ...
                $staffGramasewa->idoffice = Auth::user()->office->idoffice;
